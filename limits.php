...
class speedLimit
{
	protected function collect()
	{
		...
		$req = new rXMLRPCRequest(
			new rXMLRPCCommand( "d.multicall", array(
				...
				getCmd('cat').'=$'.getCmd("throttle.up.max").'=$'.getCmd("d.throttle_name="),
				getCmd('cat').'=$'.getCmd("throttle.down.max").'=$'.getCmd("d.throttle_name=")))
			);
		if($req->success())
		{
	...
	public function check( $req, $hash, $name, $public )
	{
		if( ($name=='slimit') && !$public )
		{
			...
			(
				$hash,
				getCmd("d.is_active="),
				getCmd('cat').'=$'.getCmd("d.stop").'=,$'.getCmd("d.throttle_name.set=").',$'.getCmd('d.start='),
				getCmd('d.throttle_name.set=')
			)));
		}
		else
		if( ($name!='slimit') && $public )
	...
	public function init()
	{
		...
		$req = new rXMLRPCRequest( array(
			new rXMLRPCCommand( "throttle.global_up.rate" ),
			new rXMLRPCCommand( "throttle.global_down.rate" ) ));
		if($req->success())
		{
			$req1 = new rXMLRPCRequest();
			if($req->val[0]==0)
				$req1->addCommand(new rXMLRPCCommand( "throttle.global_up.max.set", MAX_SPEED ));
			if($req->val[1]==0)
				$req1->addCommand(new rXMLRPCCommand( "throttle.global_down.max.set", MAX_SPEED ));
			if(!$req1->getCommandsCount() || $req1->success())
			{
				$toCorrect = $this->collect();
				$req = new rXMLRPCRequest( array(
					new rXMLRPCCommand("throttle.up", array("slimit",$MAX_UL_LIMIT."")),
					new rXMLRPCCommand("throttle.down", array("slimit",$MAX_DL_LIMIT.""))
					)
				);
...
class ratioLimit
{
	...
	protected function flush()
	{
		...
		if($req1->success())
		{
			...
			$req->addCommand(new rXMLRPCCommand("method.set", array("group.rlimit.ratio.command", getCmd("d.stop=")."; ".getCmd("d.close="))));
			return($req->success());
		}
		return(false);
	}
		...
		if( $this->sl->init() && $this->rl->init() )
		{
			$req = new rXMLRPCRequest(
				rTorrentSettings::get()->getOnInsertCommand( array('_plimits'.User::getUser(),
					getCmd('execute2').'={'.Utility::getPHP().','.dirname(__FILE__).'/update.php,"$'.
			...
			if($req->success())
			{
				...
				$req = new rXMLRPCRequest( rTorrentSettings::get()->getScheduleCommand('plimits',$trackersCheckInterval,
					getCmd('execute2').'={sh,-c,'.escapeshellarg(Utility::getPHP()).' '.escapeshellarg(dirname(__FILE__).'/check.php').' '.escapeshellarg(User::getUser()).' &}' ) );
				return( $req->success() );
			}
		}
		return(false);
