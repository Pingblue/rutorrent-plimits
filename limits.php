...
if (!defined('MAX_SPEED')) define('MAX_SPEED', 10);
        ...
        if ($this->sl->init() && $this->rl->init())
        {
            $req = new rXMLRPCRequest(
                rTorrentSettings::get()->getOnInsertCommand(array('_plimits' . User::getUser(),
                    getCmd('execute.nothrow') . '={' . Utility::getPHP() . ',' . __DIR__ . '/update.php,"$' .
            ...
            if ($req->success())
            {
                ...
                $req = new rXMLRPCRequest(
                    rTorrentSettings::get()->getScheduleCommand('plimits', $trackersCheckInterval,
                        getCmd('execute') . '={sh,-c,' . escapeshellarg(Utility::getPHP()) . ' ' . escapeshellarg(__DIR__ . '/check.php') . ' ' . escapeshellarg(User::getUser()) . ' &}')
                );
                return ($req->success());
            }
        }
